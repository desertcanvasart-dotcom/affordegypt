...
export class DatabaseStorage implements IStorage {
  ...
  async createBookingDay(dayData: InsertBookingDay): Promise<BookingDay> {
    const [day] = await db.insert(bookingDays)
      .values(dayData)
      .returning();
    return day;
  }

  async createBookingService(serviceData: InsertBookingService): Promise<BookingService> {
    const [service] = await db.insert(bookingServices)
      .values(serviceData)
      .returning();
    return service;
  }

  async updateBookingService(id: number, updates: Partial<InsertBookingService>): Promise<BookingService> {
    const [service] = await db.update(bookingServices)
      .set({ ...updates, updatedAt: new Date() })
  ...
  async calculateDayByDayQuote(bookingId: number): Promise<{ totalAmount: string; breakdown: any[] }> {
    // Get all booking days and their services
    ...
    for (const day of days) {
      const dayServices = await db.select({
        id: bookingServices.id,
        serviceId: bookingServices.serviceId,
        passengers: bookingServices.passengers,
        unitPrice: bookingServices.unitPrice,
        subtotal: bookingServices.subtotal,
        startTime: bookingServices.startTime,
        endTime: bookingServices.endTime,
        meta: bookingServices.meta,
        service: {
          id: services.id,
      ...
      .where(eq(bookingServices.bookingDayId, day.id));

      let dayTotal = 0;
      for (const service of dayServices) {
        dayTotal += parseFloat(service.subtotal || "0");
      }
    ...
    if (days.length >= 7 && totalAmount >= 1000) {
      // Multi-day discount: 3%
      const discountAmount = totalAmount * 0.03;
      totalAmount -= discountAmount;
      breakdown.push({
        type: 'discount',
        description: '7+ day booking discount (3%)',
        amount: -discountAmount.toFixed(2)
      });
    } else if (breakdown.some(day => day.services?.length >= 3)) {
      // Day-level discount: 5% for days with 3+ services
      let dayDiscountTotal = 0;
      breakdown.forEach(day => {
        if (day.services?.length >= 3) {
          const dayDiscount = parseFloat(day.dayTotal) * 0.05;
          dayDiscountTotal += dayDiscount;
          day.discount = dayDiscount.toFixed(2);
        }
      });
      
      if (dayDiscountTotal > 0) {
        totalAmount -= dayDiscountTotal;
        breakdown.push({
          type: 'discount',
          description: 'Day bundle discount (5% for days with 3+ services)',
          amount: -dayDiscountTotal.toFixed(2)
        });
      }
    }
  ...
  async getDayByDayBooking(bookingId: number): Promise<any> {
    // Get booking details
    ...
    // Get all days with their services
    const days = await db.select()
      .from(bookingDays)
      .where(eq(bookingDays.bookingId, bookingId))
      ...
      days.map(async (day) => {
        const dayServices = await db.select({
          ...
          service: {
            ...
            pricingMode: services.pricingMode,
            vehicleCategory: services.vehicleCategory,
  ...
  async updateDayByDayBooking(bookingId: number, updates: any): Promise<Booking> {
